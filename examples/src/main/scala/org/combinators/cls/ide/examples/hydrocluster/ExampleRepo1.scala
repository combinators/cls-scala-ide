...
object ExampleRepo1 {
  ...
  sealed case class VariationVTH(length: Int, index: Int) {
    ...
    def computeDoubleVines(combination: Seq[layers.LayerVTH]): Seq[Either[layers.LayerVTH, DoubleVine]] =
      combination match {
        case layers.Vine() +: layers.Vine() +: xs =>
          Left(layers.Vine()) +: Right(DoubleVine()) +: computeDoubleVines(xs)
    ...
    def apply(
      honeyCombLayers: Seq[Algebra],
      turbineLayers: Seq[Algebra],
      vineLayers: Seq[Algebra],
      doubleVineLayers: Seq[Algebra]
    ): Seq[Algebra] =
      ...
      computeDoubleVines(layerCombination)
        .foldLeft((Seq[(Either[layers.LayerVTH, DoubleVine], Int)](), (0, 0, 0, 0)))(
          {
            case (b, Left(layers.Vine())) =>
              (b._1 :+ (Left(layers.Vine()), b._2._1), (b._2._1 + 1, b._2._2, b._2._3, b._2._4))
            case (b, Right(DoubleVine())) =>
              (b._1 :+ (Right(DoubleVine()), b._2._4), (b._2._1, b._2._2, b._2._3, b._2._4 + 1))
            case (b, Left(layers.Turbine())) =>
              (b._1 :+ (Left(layers.Turbine()), b._2._2), (b._2._1, b._2._2 + 1, b._2._3, b._2._4))
            case (b, Left(layers.HoneyComb())) =>
              (b._1 :+ (Left(layers.HoneyComb()), b._2._3), (b._2._1, b._2._2, b._2._3 + 1, b._2._4))
          }
        )
        ._1
        .map {
  ...
  sealed case class VariationVT(length: Int, index: Int) {
    ...
    def computeDoubleVines(combination: Seq[layers.LayerVT]): Seq[Either[layers.LayerVT, DoubleVine]] =
      combination match {
        case layers.Vine() +: layers.Vine() +: xs =>
          Left(layers.Vine()) +: Right(DoubleVine()) +: computeDoubleVines(xs)
        case x +: xs => Left(x) +: computeDoubleVines(xs)
        ...
        .foldLeft((Seq[(Either[layers.LayerVT, DoubleVine], Int)](), (0, 0, 0)))(
          {
            ...
            case (b, Left(layers.Turbine())) =>
              (b._1 :+ (Left(layers.Turbine()), b._2._2), (b._2._1, b._2._2 + 1, b._2._3))
  ...
  sealed case class VariationVH(length: Int, index: Int) {
    ...
    def computeDoubleVines(combination: Seq[layers.LayerVH]): Seq[Either[layers.LayerVH, DoubleVine]] =
      combination match {
        case layers.Vine() +: layers.Vine() +: xs =>
          Left(layers.Vine()) +: Right(DoubleVine()) +: computeDoubleVines(xs)
        case x +: xs => Left(x) +: computeDoubleVines(xs)
        ...
        .foldLeft((Seq[(Either[layers.LayerVH, DoubleVine], Int)](), (0, 0, 0)))(
          {
            ...
            case (b, Left(layers.HoneyComb())) =>
              (b._1 :+ (Left(layers.HoneyComb()), b._2._3), (b._2._1, b._2._2, b._2._3 + 1))
          }
        )
        ._1
        .map {
  ...
  sealed case class VineAndDoubleVineTowerCombination(length: Int) {
    val semanticType
      : Type = vec(length % 2 + length / 2, vineLayer) =>: vec(length / 2, doubleVineLayer) =>: towerCombination
  ...
  sealed case class Repository(request: SynthesisRequest) {
    ...
    def addCombinatorsForRequest(
      ...
      def typeToDescriptionForVine(sensorList: Seq[String]): Seq[(String, Seq[String])] = {
        var dummyList: Seq[(String, Seq[String])] = Seq()
        if (sensorList.contains("Humidity") || sensorList.contains("Temperature")) {
          dummyList = ("Dht22", Seq("Humidity", "Temperature", "Digital")) +: dummyList
        }
        if (sensorList.contains("Uvlevel")) {
          dummyList = ("Bh1750", Seq("Uvlevel", "Digital")) +: dummyList
        }
        if (sensorList.contains("Pressure") || sensorList.contains("Altitude")) {
          dummyList = ("Bmp280", Seq("Pressure", "Altitude", "Digital")) +: dummyList
        }
        if (sensorList.contains("Water")) {
          dummyList = ("AnalogueWater", Seq("Water", "Analogue")) +: dummyList
        }
        if (sensorList.contains("SoilMoisture")) {
          dummyList = ("AnalogueMoisture", Seq("SoilMoisture", "Analogue")) +: dummyList
        }
      ...
      val vineCombinations = 0.to(vineSensorDescription.length).map(i => vineSensorDescription.combinations(i))
      val refRepWithVine = vineCombinations.foldLeft(refRepWithVariation)(
        (rep, combinationIterator) =>
          combinationIterator.foldLeft(rep)(
            (repL, ls) => {
      ...
      val honeycombCombinations =
        1.to(honeycombSensorCombDescription.length).map(i => honeycombSensorCombDescription.combinations(i))
      ...
      val turbineCombinations = 1.to(turbineSensorDescription.length).map(i => turbineSensorDescription.combinations(i))
      ...
      val simpleTopCombinations =
        1.to(simpleTopSensorDescription.length).map(i => simpleTopSensorDescription.combinations(i))
      ...
      /*
            ...
            val refRepWithActuators =
              if (request.towerActuators.contains("Pump")) {
                ...
                (request.turbineSensors.contains("Water"), request.honeycombSensors.contains("Water")) match {
                  case (false, false) =>
                    refRepWithPump
                      .addCombinator(TurbineLayerArgumentList(Seq(), Seq(("Valve", Seq("Relay")))))
                      .addCombinator(HoneyCombLayerArgumentList(Seq(), Seq(("Valve", Seq("Relay")))))
                  case (false, true) =>
                    refRepWithPump
                      .addCombinator(TurbineLayerArgumentList(Seq(), Seq(("Valve", Seq("Relay")))))
                      .addCombinator(
                        HoneyCombLayerArgumentList(Seq(("AnalogueWater", Seq("Water"))), Seq(("Valve", Seq("Relay"))))
                      )
                  case (true, false) =>
                    refRepWithPump
                      .addCombinator(
                        TurbineLayerArgumentList(Seq(("AnalogueWater", Seq("Water"))), Seq(("Valve", Seq("Relay"))))
                      )
                      .addCombinator(HoneyCombLayerArgumentList(Seq(), Seq(("Valve", Seq("Relay")))))
                  case (true, true) =>
                    refRepWithPump
                ...
                (request.turbineSensors.contains("Water"), request.honeycombSensors.contains("Water")) match {
                  case (false, false) =>
                    refRep
                      .addCombinator(TurbineLayerArgumentList(Seq(), Seq()))
                      .addCombinator(HoneyCombLayerArgumentList(Seq(), Seq()))
                  case (false, true) =>
                    refRep
                      .addCombinator(TurbineLayerArgumentList(Seq(), Seq()))
    ...
    @combinator object EmptyDoubleVineSensorAndActuatorList {
      val semanticType: Type    = list(sensorAndActuator) :&: doubleVine :&: layer
      def apply(): Seq[Algebra] = Nil
    }

    @combinator object EmptySimpleTopSensorList extends SimpleTopSensorListSig {
      override def apply(): Seq[Algebra] = Nil
    }
    ...
    @combinator object EmptyLayerArgumentsList extends FixedLayerArgumentsListSig {
      override def apply(): Seq[Algebra] = Nil
    }
  ...
  sealed case class TurbineLayerArgumentList(
    sensors: Seq[(String, Seq[String])],
    actuators: Seq[(String, Seq[String])]
  ) {
  ...
  sealed case class HoneyCombLayerArgumentList(
    sensors: Seq[(String, Seq[String])],
    actuators: Seq[(String, Seq[String])]
  ) {
  ...
  sealed case class VineSensorList(sensors: Seq[(String, Seq[String])]) extends VineSensorListSig {
    override def apply(): Seq[Algebra] =
      if (sensors.contains(("Valve", Seq("Relay")))) {
        new Algebra {
          override def result(algI: AlgebraInstance): algI.A =
            algI.actuator("Valve", Seq("Relay"))
        } +: sensors
          .filter({
            case ("Valve", _) => false
            case _            => true
